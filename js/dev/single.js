 ...
 * https://www.winwill.com
 * https://www.winwill.com/jquery/jscroll.html
 * admin@winwill.com
 */
/*--------------------------------------------------------------------------------------------------*/
...
function abookscroll() {
	$(".entry,#thecomments,#trackback_area").jscroll({
		W: "12px",
		Bar: {

			Bg: {
				Out: "#fff", Hover: "#F76C00",
				Focus: "#F76C00"
			}
		},//设置滚动条滚轴背景：鼠标离开(默认)，经过，点击
		Btn: {
			btn: true,//是否显示上下按钮 false为不显示
			uBg: { Out: "#fff", Hover: "#F76C00", Focus: "#F76C00" },//设置上按钮背景：鼠标离开(默认)，经过，点击
			dBg: { Out: "#fff", Hover: "#F76C00", Focus: "#F76C00" }//设置下按钮背景：鼠标离开(默认)，经过，点击
		}
	});
}
var commentjs = function () {
	$.getScript(themeurl + "js/comment.js");
	$('#toolBar').animate({ opacity: 'show' }, 300);
...
function abooksingleinit() {
	...
	$('.tg_t').click(function () { $(this).next('.tg_c').slideToggle(400) });
	$("pre:not([name='code'])").wrapInner("<span></span>");
	$("pre").hover(function () {
		var contentwidth = $(this).contents().width();
		var blockwidth = $(this).width();
		if (contentwidth > blockwidth) {
			$(this).stop().animate({ width: '848px' }, 250, 'swing', function () { $(this).css("overflow", "auto"); });
		}
	}, function () {
		$(this).stop().animate({ width: '740px' }, 250, 'swing', function () { $(this).css("overflow", "hidden"); });
	});
	//限制图片大小
	$('.entry p img,.comment-content p img').each(function () {
	...
	$('#comment_header li').click(function () {
		...
		$("#comment_content > ul").animate({ opacity: 'hide' }, 0).eq($('#comment_header li').index(this)).animate({ opacity: 'show' }, 500);
		return false;
	});
	$('.com_num').click(function () {
		$('#add_comment').addClass("comment_switch_active").siblings().removeClass();
		$("#comment_content > ul").animate({ opacity: 'hide' }, 0).eq(0).animate({ opacity: 'show' }, 500);
...
function commentajax() {
	...
	$('.comment-reply-link').click(function () {
		...
		$("#cancel-comment-reply-link").unbind('click').bind('click', function () {
			...
			$('#respond_area').animate({ opacity: 'hide' }, 0);
			$("#comment_area").animate({ opacity: 'show' }, 500);
	...
	$('#comment_pager a').click(function () {
		...
		$.ajax({
			...
			success: function (data) {
				...
				commentajax();
				comment_pager();
				$('#thecomments,#comment_pager').show();
